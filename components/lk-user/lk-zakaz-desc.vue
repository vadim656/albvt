  ...
  >
    <div class="grid grid-cols-3 items-center">
      ...
      <div class="flex items-end gap-1 col-span-1">
        ...
        <!-- <span class="text-[14px] text-[#8B8B8B]">{{
          order.node.date.substring(10, 0)
        }}</span> -->
      </div>
      <!-- <button
    ...
    <div class="grid grid-cols-3 grid-rows-1 gap-y-4">
      ...
      <div class="col-span-1 w-full flex justify-center">
        <span
          v-if="order.node.status == 'PROCESSING'"
        ...
        >
        <span
          v-else-if="order.node.status == 'CANCELLED'"
        ...
        <span
          v-else-if="order.node.status == 'COMPLETED'"
        ...
        >
          Оплатить
          {{
            parseInt(order.node.total.replace('₽', '')).toLocaleString('ru-RU')
          }}
          ₽
        </button>
      </div> -->
  ...
  computed: {
    sortedDataDay: function () {
      ...
      const dayValue = days.map(item =>
        parseInt(order_data.product.node.attributes.edges[0].node.options[0])
      )

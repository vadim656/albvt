  ...
  <div class="flex flex-col gap-[20px] ">
    ...
    <div class="bg-white shadow-md rounded-[5px] p-[10px]">
      <div v-if="viewAllAnalizes == false">
        <div
          class="grid grid-cols-[2fr,13fr,2fr,3fr] w-full border-b border-b-[#C3C3C3] gap-[20px] py-[12px]"
        >
          <span class="">Код</span>
        ...
        <div 
        class="flex flex-col w-full  mt-[12px] "
        >
          <!-- one analiz  -->
          <div
            v-for="item in itemsSlice"
            :key="item.sku"
            class="grid grid-cols-[2fr,13fr,2fr,3fr] gap-[20px] items-center "
          >
            ...
            >
              <nuxt-link
                class=" test-text anime cursor-pointer hover:text-main h-full"
                :to="`/all-analyzes/` + item.id"
                :title="item.name"
                >{{ item.name }}</nuxt-link
              >
